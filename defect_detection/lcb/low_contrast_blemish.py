"""
 LCB performs grayspot/blemish
 defects detection on flat field images

 mainly to find out if there is/are huge dust/damage along the optical path (Lens / Infrared Cut-Filter Glass / CMOS sensor)
 ...
 INPUT:
   ...
   threshold:          threshold limit for Heat_Map

 OUTPUT: LCB Heat_Map Image
...
def lcb(IDraw, bayerformat="rggb", pedestal=64, bitdepth=10, mode=2, roiSize=[13, 13], filterWidth=9, threshold=3.5,
        interpolation=True, exceed2maxval=True, compensation=False, custom_source=None, LCBraw=False):
    if custom_source is None:
        if not LCBraw:
            IDbayer = preprocess(IDraw, outputformat="bayer", mode=mode, more_precise=True)
        else:
            IDbayer = preprocess(IDraw, outputformat="raw", mode=mode, more_precise=True)
    ...
    if exceed2maxval:
        ...
        # output_image = (output_image * 50).astype(np.uint8)
    ...
    if not LCB_Raw:
        ...
        for j in range(bin_size[1]):  # width
            for k in range(bin_size[0]):  # height
                ...
                y2 = rows[k + 1]
                cords = [x1, y1, x2, y2]  # debug for cordinates
                roiData = ID_fullRes[y1:y2, x1:x2]

                if block_stat is "mean":
                    ID_binned[k, j] = np.mean(roiData)
                elif block_stat is "median":
                    ID_binned[k, j] = np.median(roiData)
                elif block_stat is "mode":
                    ID_binned[k, j] = np.searchsorted(np.unique(roiData), roiData.flat)
